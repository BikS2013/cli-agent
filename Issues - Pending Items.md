...
### [MEDIUM] No way to restrict a wrapped `--tool` binary to specific subcommands (bare auto-entry defeats `argv-regex:`)
- Surfaced 2026-07-03 while fixing the auto-allowlist documentation contradiction.
- Allowlist entries are OR'd (`src/agent/tools/bash/allowlist.ts:35-43`: any matching entry allows the call), and every `--tool <name>` is auto-added as a bare-name entry (`src/config/agent-config.ts:1120`). A bare-name entry matches any argv of that binary, so an `argv-regex:^git (status|…)` rule cannot restrict a binary that is also wrapped — the combination "capability document + subcommand restriction" is unachievable today.
- Impact: users must choose between (a) wrapping a CLI (full capability doc, but full binary authority via `bash_run`) and (b) restricting it via `argv-regex:` only (no capability doc). Compounds the existing "[MEDIUM] `bash_run` read-only mode is advisory" item: for wrapped high-blast-radius CLIs (`kubectl`, `aws`) there is no enforcement-grade narrowing at all.
- Suggested fix (enhancement): let an explicit `argv-regex:` entry for a binary *override* (not OR with) the auto-added bare entry — e.g. suppress the auto-add when the user supplies any `argv-regex:` rule whose pattern targets that binary, or add a `--tool-restrict <name>=<regex>` flag. Update the allowlist matcher docs/tests accordingly.

### [MEDIUM] Bash `bash_run` is fail-OPEN when the allowlist is unconfigured (user-directed 2026-07-04)
- Per direct user instruction, an unconfigured bash allowlist now means UNRESTRICTED: `bash_run` executes any binary on `PATH` (previously fail-closed — the tool was not even registered). Implemented in `src/agent/tools/bash/allowlist.ts`, `registry.ts`, `agent-tools/permissions.ts`, `run-tool.ts`, `list-allowed-tool.ts`, `system-prompt.ts`; recorded in the memory note `bash-allowlist-fail-open` and the 2026-07-04 design entry.
- Impact: this reverses the prior security invariant (FR-AGT-017). A user who launches cli-agent without configuring an allowlist (and the model, via prompt injection or error) can run arbitrary commands. Mitigations that remain: one stderr notice at startup, the `--allow-mutations` gate for mutating operations, the `cwd` sandbox, `execFile` semantics, and env stripping. The system prompt instructs restraint in unrestricted mode, but that is advisory (mirrors the existing "[MEDIUM] `bash_run` read-only mode is advisory" item).
- Tracking note (not a defect to fix — the behavior is intentional): revisit if the default should instead be opt-in via an explicit `--bash-allow-all` flag so the fail-open posture is a conscious per-run choice rather than the zero-config default. Surface this option to the user at the next review.
...
### Done — Profile tier sat at different precedence positions for pinnable knobs vs tool-group toggles (resolved 2026-07-04 by Plan 015)
- **Symptom** (registered 2026-07-03 as [LOW]): profile `cliParams` (provider, model, temperature, allowMutations, …) outranked `config.json` (tier-5 insertion), while the three tool-group toggles (`tools.builtin`/`tools.composites`/`tools.agentTools`) put the profile *below* `config.json` (`resolveToolGroupToggle`). Both orders were individually documented, but the asymmetry was easy to get wrong — `agent-competency-levels.md` presented one uniform (and for toggles incorrect) chain until 2026-07-03.
- **Resolution**: exactly the fix the item's own "Suggested fix" anticipated — Plan 015 made `--mode` the single tool-group knob, resolved through the uniform pinnable chain (CLI > `CLI_AGENT_MODE` > profile `cliParams.mode` > `config.json` `mode` > default `composite`), and hard-removed the group-toggle chain (`resolveToolGroupToggle` deleted from `src/config/agent-config.ts`; the legacy flag/env/config/profile surfaces now fail fast with a migration hint). The profile tier now has ONE position for every knob.
- **Reference**: `docs/design/plan-015-cli-mode-simplification.md` (Step 20); design entry "2026-07-04 — CLI Mode Simplification" in `docs/design/project-design.md`; FR-MODE-3 / NFR-MODE-2 in `docs/design/project-functions.md`.

### Done — Documentation contradicted code on `--tool` auto-allowlisting; wrong env var and inverted precedence chain fixed (2026-07-03)
- **Symptom**: Three documentation defects. (1) `docs/guides/enabling-write-capabilities.md` claimed binaries attached with `--tool` are "**not** automatically added" to the bash allowlist, contradicting `docs/tools/cli-agent.md`, `docs/design/project-functions.md` (FR: "Each declared tool binary must be automatically added to the bash allowlist at startup"), and the code (`src/config/agent-config.ts:1120` merges wrapped-tool names into `mergedBashAllow`). Its recipes carried redundant `--bash-allow git`/`gh` flags, and its "fine-grained" recipe (`--tool git` + `argv-regex:` restriction) was actively misleading — the auto-added bare `git` entry OR-wins over any regex (`src/agent/tools/bash/allowlist.ts:35-43`), so the restriction never applied. (2) The same guide referenced a nonexistent `AGENT_FILE_ROOT` env var (the real one is `FILE_EDIT_ROOT`, `src/config/agent-config.ts:1155`). (3) `docs/guides/agent-competency-levels.md` showed the tool-group-toggle precedence chain with profile above `config.json`; the code puts `config.json` above profile for those toggles.
- **Fix**: Corrected `enabling-write-capabilities.md` throughout (Switch 2 intro, mental-model table, quick recipes 2–4, Examples B/C, troubleshooting bullet, `FILE_EDIT_ROOT`), adding an explicit "entries are OR'd" warning and rewriting the fine-grained recipes to allow the binary *only* via `argv-regex:` (unwrapped) with the trade-off documented. Fixed both precedence listings in `agent-competency-levels.md` (pinnable-knob chain vs group-toggle chain) and extended its allowlist tip with the OR-semantics caveat. Added the same caveat to `docs/design/configuration-guide.md` under the argv-regex worked example.
- **Also fixed — CLI `--help` and TUI `/help` carried the same stale claims**: `src/cli.ts` described `--allow-mutations` with the pre-plan-012 tool names (`file_write, file_edit, file_append`), `--tool` did not mention auto-allowlisting, and `--bash-allow` didn't name the two entry kinds; `src/tui/slash/allow-mutations.ts` used the old tool names in its `/help` summary. All four strings updated (`agt_file_write/edit/append, agt_multiedit, agt_patch`; "auto-added to the bash allowlist"; "bare binary names or argv-regex:<pattern> rules"), and the byte-stability baseline `test_scripts/baselines/help-no-treat-as-tool.txt` was consciously re-recorded per NFR-CMP-001.
- **Validation**: `grep` sweep across `docs/` and `README.md` confirms all remaining "auto-added" statements agree with the code; `AGENT_FILE_ROOT` no longer appears anywhere; the two corrected chains match `resolveAgentTools`/`resolveToolGroupToggle` and the tier-5 `cliParams` insertion in `src/config/agent-config.ts`; `npm run build` then `vitest run src/cli-help-baseline.spec.ts` (2/2) and `vitest run src/tui/slash` (53/53) pass.
- **Follow-ups registered as new pending items**: "[MEDIUM] No way to restrict a wrapped `--tool` binary to specific subcommands" and "[LOW] Profile tier sits at different precedence positions".
